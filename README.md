# simulated-evolution
A curated list of resources for simulated evolution, genetic programming and evolutionary programming

## Distributed Systems
 [Example How to use Apache Ignite for genetic algorithms](https://apacheignite.readme.io/docs/genetic-algorithms)

## Java Libraries

## JavaScript Libraries

## Python Libraries
- [Distributed Evolutionary Algorithms in Python (DEAP)](https://github.com/DEAP/deap)
- [gplearn](https://github.com/trevorstephens/gplearn)


## Blog Posts
- [Genetic Algorithm Implementation in Python](https://towardsdatascience.com/genetic-algorithm-implementation-in-python-5ab67bb124a6)
- [Was Darwin a Great Scientist? How evolution taught us the 'genetic algorithm' (Python)](https://blog.sicara.com/getting-started-genetic-algorithms-python-tutorial-81ffa1dd72f9)
- [Python Genetic Algorithms With Artificial Intelligence](https://medium.com/@rinu.gour123/python-genetic-algorithms-with-artificial-intelligence-b8d0c7db60ac)
- [Using Genetic Programming to evolve Trading Strategies](http://www.turingfinance.com/using-genetic-programming-to-evolve-security-analysis-decision-trees/)
- [Symbolic Regression, Genetic Programming… or if Kepler had R](https://www.r-bloggers.com/symbolic-regression-genetic-programming-or-if-kepler-had-r/)
- [Learning Machines - Evolution works!](http://blog.ephorie.de/evolution-works)

## Books
- [Field Guide to Genetic Programming](http://www.gp-field-guide.org.uk/)


## The Theory
- [Genetic Programming](http://geneticprogramming.com/)

## News
- [MIT Tech Review: Evolutionary algorithm outperforms deep-learning machines at video games](https://www.technologyreview.com/s/611568/evolutionary-algorithm-outperforms-deep-learning-machines-at-video-games/)

## R Libraries
- [gramEvol](https://cran.r-project.org/web/packages/gramEvol/index.html)
